window.WashMeetingHistoryUI = (function(){
  ...
  function renderList(records, selectedId){
    ...
    return records.map(record=>{const selected=record.id===selectedId;const activeStyle=selected?'background:rgba(14,165,233,0.08);border:1px solid rgba(14,165,233,0.22);border-left:4px solid #0ea5e9;':'';return `<article class="historial-card ${selected?'active':''}" style="${activeStyle}" onclick="WashMeetingHistory.openDetail('${esc(record.id)}')">
      <div class="historial-class-line ${lineClass(record.classification)}" style="${selected?'background:#0ea5e9;width:4px':''}"></div>
