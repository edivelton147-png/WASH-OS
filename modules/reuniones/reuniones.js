...
(function(){
  ...
  async function loadMeetingPrompts(){const loaded=window.WashPromptLoader&&window.WashPromptLoader.loadPrompt?await window.WashPromptLoader.loadPrompt(getPromptUrl()):{};return {...PROMPT_FALLBACK,...loaded};}
  ...
  async function sendTasksToManager(tasks,input){
    ...
    for(const task of source){
      ...
      const dueDate = task.date||task.fecha||'';
      const newTask={
        id:Date.now()+added,
        ...
        cierre:(dueDate&&dueDate!=='No especificado'?`${dueDate}T18:00:00`:''),
