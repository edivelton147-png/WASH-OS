...
(function(){
  ...
  function combineDateWithBaseHour(dateText,base){const end=new Date(base.getTime()+60*60000);if(dateText){const [y,m,d]=dateText.split('-').map(Number);end.setFullYear(y,m-1,d);}return end;}
  ...
  window.WashGestorSuggestions={
    receive(detail){
      ...
      state.suggestions=tasks.map((item,i)=>({id:`sg-${Date.now()}-${i}`,checked:true,title:taskText(item),desc:taskDesc(item),category:taskCategory(item),priority:taskPriority(item),date:taskDate(item,detail?.record),endDate:'',link:taskLink(item,detail?.record)}));
    ...
    addSelected(){
      ...
      state.suggestions.forEach(s=>{
        ...
        const start=combineDateWithSmartHour(date);
        const end=endDate?combineDateWithBaseHour(endDate,start):new Date(start.getTime()+60*60000);
