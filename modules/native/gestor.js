...
(function(){
  ...
  function renderSuggestions(){
    ...
    return `<div id="gestor-suggested-tray" style="background:#fff;border:1px solid #bfdbfe;border-left:4px solid #1A75BB;border-radius:12px;padding:14px;margin-bottom:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:8px">${state.suggestions.map((s,i)=>`<div data-suggestion="${s.id}" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px"><div style="display:grid;grid-template-columns:24px 1.4fr .5fr .55fr 1fr auto;gap:8px;align-items:center"><input type="checkbox" id="sg-check-${s.id}" ${s.checked?'checked':''} style="width:16px;height:16px"><input id="sg-title-${s.id}" value="${esc(s.title)}" style="font-size:12px" placeholder="Título"><select id="sg-pri-${s.id}" style="font-size:12px"><option ${s.priority==='Alta'?'selected':''}>Alta</option><option ${s.priority==='Media'?'selected':''}>Media</option><option ${s.priority==='Baja'?'selected':''}>Baja</option></select><input id="sg-date-${s.id}" type="date" value="${esc(s.date)}" style="font-size:12px"><input id="sg-link-${s.id}" value="${esc(s.link)}" placeholder="Enlace/documento" style="font-size:12px"><button onclick="WashGestorSuggestions.discard('${s.id}')" style="background:#fff;color:#dc2626;border:1px solid #fecaca;padding:6px 10px;font-size:12px;border-radius:8px">Descartar</button></div><textarea id="sg-desc-${s.id}" rows="2" style="margin-top:8px;font-size:12px;resize:vertical" placeholder="Descripción">${esc(s.desc)}</textarea></div>`).join('')}</div>
    </div>`;
  }
