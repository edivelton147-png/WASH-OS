(function(){
  ...
  function safePath(path){
    if(typeof window!=='undefined' && typeof window.resolveAppPath==='function') return window.resolveAppPath(path);
    return path;
  }
  ...
  function normalizeRoute(route){
    ...
    return {
      ...
      manualOutputs: Array.isArray(route.manualOutputs) && route.manualOutputs.length ? route.manualOutputs : DEFAULT_ROUTE.manualOutputs
    };
  }
  ...
  async function loadRoutesOnce(){
    ...
    if(!routesPromise){
      routesPromise = fetch(safePath('/config/ai-routes.json'), { cache:'no-cache' })
        .then(response=>{
  ...
  if(typeof module!=='undefined') module.exports = { routeAI, loadRoutesOnce };
})();
